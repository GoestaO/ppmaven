...
public class ExcelGenerator {
     ...
     *
     * @param filename Die Datei, in der der Report gespeichert werden soll
     * @param leistungsreport Das Resultat der Datenbankabfrage.
     */
    public void createLeistungsreportNoUser(File filename, List<LeistungsreportNoUser> leistungsreport) {
        Workbook wb = new XSSFWorkbook();
        Sheet sheet = wb.createSheet();
        Row headerRow = sheet.createRow(0);
        ...
        int rownum = 1;
        for (LeistungsreportNoUser lr : leistungsreport) {
            Row row = sheet.createRow(rownum++);

            Cell nameCell = row.createCell(0);

            Cell configCell = row.createCell(1);
            configCell.setCellValue(lr.getConfig());

            Cell appdomainCell = row.createCell(2);
            appdomainCell.setCellValue(lr.getAppdomainId());

            Cell partnerCell = row.createCell(3);
            partnerCell.setCellValue(lr.getPartnerId());

            Cell cgPathCell = row.createCell(4);
            cgPathCell.setCellValue(lr.getCgPath());

            Cell seasonCell = row.createCell(5);
            seasonCell.setCellValue(lr.getSaison());

            Cell dateCell = row.createCell(6);
            dateCell.setCellValue(lr.getDatum());
            dateCell.setCellStyle(dateCellStyle);

            Cell statusCell = row.createCell(7);
            statusCell.setCellValue(lr.getStatus());

            Cell comment1Cell = row.createCell(8);
            comment1Cell.setCellValue(lr.getBemerkung1());

            Cell comment2Cell = row.createCell(9);
            comment2Cell.setCellValue(lr.getBemerkung2());

            Cell comment3Cell = row.createCell(10);
            comment3Cell.setCellValue(lr.getBemerkung3());

            Cell commentKAMCell = row.createCell(11);
            commentKAMCell.setCellValue(lr.getBemerkungKAM());
        }
        for (int i = 0; i < 12; i++) {
            sheet.autoSizeColumn(i);
        }
